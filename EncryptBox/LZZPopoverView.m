...
@interface LZZPopoverView ()

@property (nonatomic, strong) UIControl *blackOverlay;
@property (nonatomic, weak) UIView *containerView;
@property (nonatomic, assign, readwrite) LZZPopoverPosition popoverPosition;
...
@implementation LZZPopoverView

+ (instancetype)popover
{
    return [[LZZPopoverView alloc] init];
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.sideEdge = 10.0;
        self.maskType = LZZPopoverMaskTypeBlack;
...
- (void)_setup
{
    ...
    CGPoint LZZ_lastAnchor = self.layer.anchorPoint;
    self.layer.anchorPoint = anchorPoint;
    self.layer.position = CGPointMake(self.layer.position.x+(anchorPoint.x-LZZ_lastAnchor.x)*self.layer.bounds.size.width, self.layer.position.y+(anchorPoint.y-LZZ_lastAnchor.y)*self.layer.bounds.size.height);\
...
{
    NSAssert((CGRectGetWidth(contentView.bounds)>0&&CGRectGetHeight(contentView.bounds)>0), @"LZZPopover contentView bounds.size should not be zero");
    NSAssert((CGRectGetWidth(containerView.bounds)>0&&CGRectGetHeight(containerView.bounds)>0), @"LZZPopover containerView bounds.size should not be zero");
    NSAssert(CGRectGetWidth(containerView.bounds)>=CGRectGetWidth(contentView.bounds), @"LZZPopover containerView width should be wider than contentView width");
    ...
    switch (self.maskType) {
        case LZZPopoverMaskTypeBlack:
            maskColor = [UIColor colorWithWhite:0.0 alpha:0.2];
            break;
        case LZZPopoverMaskTypeNone:
...
- (void)showAtView:(UIView *)atView popoverPostion:(LZZPopoverPosition)position withContentView:(UIView *)contentView inView:(UIView *)containerView
{
    ...
    NSAssert((upCanContain||downCanContain), @"LZZPopover no place for the popover show, check atView frame %@ check contentView bounds %@ and containerView's bounds %@", NSStringFromCGRect(atViewFrame), NSStringFromCGRect(contentView.bounds), NSStringFromCGRect(containerView.bounds));
    
    
    CGPoint atPoint = CGPointMake(CGRectGetMidX(atViewFrame), 0);
    LZZPopoverPosition dxP;
    if (upCanContain) {
        dxP = LZZPopoverPositionUp;
        atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
    }else {
        dxP = LZZPopoverPositionDown;
    ...
    if (upCanContain && downCanContain) {
        ...
        if (position!=0) {
            useUp = position == LZZPopoverPositionUp ? YES : NO;
        }
        if (useUp) {
            dxP = LZZPopoverPositionUp;
            atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
